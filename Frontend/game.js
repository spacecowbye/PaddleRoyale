...
function drawStartScreen() {
    ...
    c.font = '16px Inter';
    c.fillText('Use W/S or Arrow Keys to move', CANVAS_WIDTH/2, CANVAS_HEIGHT/2 + 40);
}
...
document.addEventListener('keyup', (event) => {
    if ((event.key === 'w' || event.key === 'W' || event.key === "ArrowUp") && playerPaddle.dy < 0) {
        playerPaddle.dy = 0;
    }
    if ((event.key === 's' || event.key === 'S' || event.key === "ArrowDown") && playerPaddle.dy > 0) {
        playerPaddle.dy = 0;
    }
});
